...
function Player:update(dt)
    ...
    if self.map:tileAt(self.x, self.y + 1).id == FLAG_BEG or self.map:tileAt(self.x, self.y - 1).id == FLAG_BEG then
      gameState = 'done'
    end
end
...
--function Player:collides(FLAGPOLE_BTM)
    -- first, check to see if the left edge of either is farther to the right
    -- than the right edge of the other
  --  if self.x > FLAGPOLE_BTM.x + FLAGPOLE_BTM.width or FLAGPOLE_BTM.x > self.x + self.width then
  --      return false
--    end

    -- then check to see if the bottom edge of either is higher than the top
    -- edge of the other
  --  if self.y > FLAGPOLE_BTM.y + FLAGPOLE_BTM.height or FLAGPOLE_BTM.y > self.y + self.height then
--        return false
--    end
